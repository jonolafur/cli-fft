...
void spectralAveraging( fftOptions& opt, fftw_vector& fft_vec )
{
   ...
   fftw_vector short_vec( n, 0.0, 0.0 ), result_vec( n, 0.0, 0.0 );

   double overlap = opt.welchOverlap(n);
   size_t i=0;
   size_t j=0;
   
   while( j < N )
   {
      short_vec[i][0] = fft_vec[j][0];
      short_vec[i][1] = fft_vec[j][1];
      ...
      if( i == n )
      {
         ...
         {
            if(opt.averagePower())
               fftw_add( result_vec[k], short_vec.power( k ) );
            else if(opt.averageMagnitude())
               fftw_add( result_vec[k], short_vec.abs( k ) );
            else if( opt.averageLinear() )
               fftw_add( result_vec[k], short_vec[k] );
         }
      }
   }
   ...
   if(opt.averagePower())
      ...
      {
         fft_vec[i][0] = sqrt(fft_vec[i][0]);
         fft_vec[i][1] = sqrt(fft_vec[i][1]);
      }
...
{
   if(opt.useRectangular())
      return;
   
   if(opt.useBartlett())
   {
      bartlett bw;
      bw.applyWindow(fft_vec);
      return;
   }
